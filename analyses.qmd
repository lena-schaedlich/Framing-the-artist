...
for(artist in levels(aesthemos_df$Artist)){
  ...
  ggsave(glue("Figures/response-aesthemos-{artist}.png"), units = "cm", width = 32, height = 24)
}
```
...
for(artist in levels(aesthetics_df$Artist)){
  ...
  ggsave(glue("Figures/response-aesthetics-{artist}.png"), units = "cm", width = 24, height = 8)
}
```
